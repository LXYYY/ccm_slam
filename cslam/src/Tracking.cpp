...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat& im,
  ...
  if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
    mCurrentFrame.reset(new Frame(mImGray, timestamp, mpIniORBextractor,
                                  mpORBVocabulary, mK, mDistCoef, mClientId));
  else {
    mCurrentFrame.reset(new Frame(mImGray, timestamp, mpORBextractor,
                                  mpORBVocabulary, mK, mDistCoef, mClientId));
  }
...
void Tracking::Track() {
  ...
  // Comm Mutex cannot be acquired here. In case of wrong initialization, there
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    // Get Communicator Mutex -> Comm cannot publish. Assure no publishing
    // whilst changing data
    ...
    if (mState == OK) {
      ...
      if (mVelocity.empty() ||
          mCurrentFrame->mId.first < mLastRelocFrameId.first + 2) {
...
void Tracking::CreateInitialMapMonocular() {
  // Get Communicator Mutex -> Comm cannot publish. Assure no publishing whilst
  // changing data
  while (!mpCC->LockTracking()) {
    usleep(params::timings::miLockSleep);
  }
  ...
  // Create KeyFrames
  kfptr pKFini{new KeyFrame(*mInitialFrame, mpMap, mpKeyFrameDB, mpComm,
                            eSystemState::CLIENT, -1)};
  kfptr pKFcur{new KeyFrame(*mCurrentFrame, mpMap, mpKeyFrameDB, mpComm,
                            eSystemState::CLIENT, -1)};
  ...
  for (size_t i = 0; i < mvIniMatches.size(); i++) {
    ...
    mpptr pMP{new MapPoint(worldPos, pKFcur, mpMap, mClientId, mpComm,
                           eSystemState::CLIENT, -1)};
...
void Tracking::CheckReplacedInLastFrame() {
  for (int i = 0; i < mLastFrame->N; i++) {
    ...
    if (pMP) {
      ...
      if (pRep) {
        vector<mpptr>::iterator vit =
            std::find(mLastFrame->mvpMapPoints.begin(),
                      mLastFrame->mvpMapPoints.end(), pRep);
...
bool Tracking::TrackWithMotionModel() {
  ...
  if (nmatches <
      params::tracking::miTrackWithMotionModelInlierThresSearch)  // 20
    return false;
  ...
  return nmatchesMap >=
         params::tracking::miTrackWithMotionModelInlierThresOpt;  // 10
}
...
bool Tracking::NeedNewKeyFrame() {
  ...
  if (mCurrentFrame->mId.first <
          mLastRelocFrameId.first + params::tracking::miMaxFrames &&
      nKFs > params::tracking::miMaxFrames) {
    return false;
  }
  ...
  const bool c1a = mCurrentFrame->mId.first >=
                   mLastKeyFrameId.first + params::tracking::miMaxFrames;
...
void Tracking::CreateNewKeyFrame() {
  ...
  kfptr pKF{new KeyFrame(*mCurrentFrame, mpMap, mpKeyFrameDB, mpComm,
                         eSystemState::CLIENT, -1)};

  std::vector<mpptr> vpM = pKF->GetMapPointMatches();
  for (vector<mpptr>::const_iterator vit = vpM.begin(); vit != vpM.end();
       ++vit) {
    mpptr pMPi = *vit;
...
void Tracking::SearchLocalPoints() {
  ...
  for (vector<mpptr>::iterator vit = mCurrentFrame->mvpMapPoints.begin(),
  ...
  for (vector<mpptr>::iterator vit = mvpLocalMapPoints.begin(),
...
void Tracking::UpdateLocalPoints() {
  ...
  for (vector<kfptr>::const_iterator itKF = mvpLocalKeyFrames.begin(),
    ...
    for (vector<mpptr>::const_iterator itMP = vpMPs.begin(),
                                       itEndMP = vpMPs.end();
         itMP != itEndMP; itMP++) {
      mpptr pMP = *itMP;
      if (!pMP) {
      ...
      if (pMP->mTrackReferenceForFrame == mCurrentFrame->mId) continue;
      if (!pMP->isBad()) {
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  for (int i = 0; i < mCurrentFrame->N; i++) {
    if (mCurrentFrame->mvpMapPoints[i]) {
      ...
      if (!pMP->isBad()) {
        ...
        for (map<kfptr, size_t>::const_iterator it = observations.begin(),
  ...
  for (map<kfptr, int>::const_iterator it = keyframeCounter.begin(),
  ...
  for (vector<kfptr>::const_iterator itKF = mvpLocalKeyFrames.begin(),
    ...
    for (vector<kfptr>::const_iterator itNeighKF = vNeighs.begin(),
    ...
    for (set<kfptr>::const_iterator sit = spChilds.begin(),
                                    send = spChilds.end();
         sit != send; sit++) {
      kfptr pChildKF = *sit;
      if (!pChildKF->isBad()) {
...
Tracking::kfptr Tracking::GetReferenceKF() {
  // Works w/o mutex, since tracking is locked when this method is called by
  // comm

  if (!mpCC->IsTrackingLocked())
    cout << "\033[1;33m!!! WARN !!!\033[0m " << __func__ << ":" << __LINE__
