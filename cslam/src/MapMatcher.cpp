...
MapMatcher::MapMatcher(ros::NodeHandle Nh, ros::NodeHandle NhPrivate, dbptr pDB,
  ...
  mPubMarker =
      mNh.advertise<visualization_msgs::Marker>("MapMatcherMarkers", 10);
...
bool MapMatcher::DetectLoop() {
  ...
  if (!mpCurrMap) {
    cout << ": In \"MapMatcher::DetectLoop()\": mpCurrMap is nullptr -> KF not "
  ...
  vector<kfptr> vpCandidateKFs =
      mpKFDB->DetectMapMatchCandidates(mpCurrentKF, minScore, mpCurrMap);
  ...
  for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
    ...
    for (size_t iG = 0, iendG = mmvConsistentGroups[mpCurrMap].size();
         iG < iendG; iG++) {
...
bool MapMatcher::ComputeSim3() {
  ...
  for (vector<kfptr>::iterator vit = vpLoopConnectedKFs.begin();
    ...
    for (size_t i = 0, iend = vpMapPoints.size(); i < iend; i++) {
      ...
      if (pMP) {
        if (!pMP->isBad() &&
            pMP->mLoopPointForKF_MM != mpCurrentKF->mId)  // ID Tag
        {
...
void MapMatcher::CorrectLoop() {
  ...
  for (set<size_t>::iterator sit = suAssCLientsCurr.begin();
       sit != suAssCLientsCurr.end(); ++sit) {
    size_t idc = *sit;
    for (set<size_t>::iterator sit2 = suAssCLientsMatch.begin();
         sit2 != suAssCLientsMatch.end(); ++sit2) {
      size_t idm = *sit2;

      if (idc == idm)
        cout << "\033[1;31m!!! ERROR !!!\033[0m In "
                "\"MapMatcher::CorrectLoop()\": Associated Clients of matched "
                "and current map intersect"
             << endl;

      mMatchMatrix.at<uint16_t>(idc, idm) =
          mMatchMatrix.at<uint16_t>(idc, idm) + 1;
      mMatchMatrix.at<uint16_t>(idm, idc) =
          mMatchMatrix.at<uint16_t>(idm, idc) + 1;
    }
  }

  if (mpCurrentKF->mId.second == mpMatchedKF->mId.second)
    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMatcher::CorrectLoop()\": "
            "Matched KFs belong to same client"
         << endl;
  if (!mpCurrMap->msuAssClients.count(mpCurrentKF->mId.second))
    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMatcher::CorrectLoop()\": "
            "Current KFs does not belong to current map"
         << endl;
  if (mpCurrMap->msuAssClients.count(mpMatchedKF->mId.second))
    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMatcher::CorrectLoop()\": "
            "Matched KFs belongs to current map"
         << endl;

  if (params::vis::mbActive) PublishLoopEdges();

  mapptr pMatchedMap = mpMatchedKF->GetMapptr();

  MapMatchHit MMH(mpCurrentKF, mpMatchedKF, mg2oScw, mvpLoopMapPoints,
  ...
  if (mFoundMatches[mpCurrMap][pMatchedMap].size() >= 1) {
    ...
    mapptr pMergedMap =
        mpMapMerger->MergeMaps(mpCurrMap, pMatchedMap, vMatches);
  }
...
void MapMatcher::PublishLoopEdges() {
  ...
  try {
    ...
    mTfListen.lookupTransform("world", FrameIdMatched, ros::Time(0),
                              Tf_W_Matched);
  ...
  tf::Point PTfCurr{
      params::vis::mfScaleFactor * ((double)(TCurr.at<float>(0, 3))),
      params::vis::mfScaleFactor * ((double)(TCurr.at<float>(1, 3))),
      params::vis::mfScaleFactor * ((double)(TCurr.at<float>(2, 3)))};
  tf::Point PTfMatch{
      params::vis::mfScaleFactor * ((double)(TMatch.at<float>(0, 3))),
      params::vis::mfScaleFactor * ((double)(TMatch.at<float>(1, 3))),
      params::vis::mfScaleFactor * ((double)(TMatch.at<float>(2, 3)))};
...
void MapMatcher::EraseKFs(vector<kfptr> vpKFs) {
  ...
  for (vector<kfptr>::iterator vit = vpKFs.begin(); vit != vpKFs.end(); ++vit) {
    kfptr pKF = *vit;
    std::list<kfptr>::iterator lit =
        find(mlKfInQueue.begin(), mlKfInQueue.end(), pKF);
