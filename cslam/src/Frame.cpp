...
Frame::Frame(const cv::Mat& imGray, const double& timeStamp,
             extractorptr pExtractor, vocptr pVoc, cv::Mat& K,
             cv::Mat& distCoef, size_t ClientId)
...
void Frame::ComputeStereoMatches() {
  ...
  for (int iL = 0; iL < N; iL++) {
    ...
    if (bestDist < thOrbDist) {
      ...
      const int w = 5;
      cv::Mat IL = mpORBextractor->mvImagePyramid[kpL.octave]
      ...
      if (iniu < 0 || endu >= mpORBextractor->mvImagePyramid[kpL.octave].cols)
        continue;
