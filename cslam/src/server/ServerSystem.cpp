...
ServerSystem::ServerSystem(ros::NodeHandle Nh, ros::NodeHandle NhPrivate,
                           const string& strVocFile)
    : mNh(Nh),
      mNhPrivate(NhPrivate),
      mMaxClients(4),
      mpUID(new estd::UniqueIdDispenser()) {
  params::ShowParams();
  ...
  mServiceSavemap = mNh.advertiseService("ccmslam_savemap",
                                         &ServerSystem::CallbackSaveMap, this);
...
bool ServerSystem::CallbackSaveMap(ccmslam::ServiceSaveMap::Request& req,
...
void ServerSystem::InitializeClients() {
  ...
  mpClient0.reset(new ClientHandler(mNh, mNhPrivate, mpVoc, mpKFDB, mpMap0, 0,
                                    mpUID, eSystemState::SERVER, string(),
                                    mpViewer, bLoadMapFromFile));
  ...
  // Client 1
  if (mNumOfClients > 1) {
    mpClient1.reset(new ClientHandler(mNh, mNhPrivate, mpVoc, mpKFDB, mpMap1, 1,
                                      mpUID, eSystemState::SERVER, string(),
                                      mpViewer));
#ifdef LOGGING
  ...
  // Client 2
  if (mNumOfClients > 2) {
    mpClient2.reset(new ClientHandler(mNh, mNhPrivate, mpVoc, mpKFDB, mpMap2, 2,
                                      mpUID, eSystemState::SERVER, string(),
                                      mpViewer));
#ifdef LOGGING
  ...
  // Client 3
  if (mNumOfClients > 3) {
    mpClient3.reset(new ClientHandler(mNh, mNhPrivate, mpVoc, mpKFDB, mpMap3, 3,
                                      mpUID, eSystemState::SERVER, string(),
                                      mpViewer));
#ifdef LOGGING
  ...
  if (mNumOfClients > mMaxClients) {
    cout << "\033[1;33m!!! WARN !!!\033[0m Maximum number of clients is "
         << mMaxClients << endl;
  }
...
void ServerSystem::InitializeMapMatcher() {
  mpMapMatcher.reset(new MapMatcher(mNh, mNhPrivate, mpKFDB, mpVoc, mpMap0,
                                    mpMap1, mpMap2, mpMap3,
                                    mSensor != eSensor::MONOCULAR));
...
void ServerSystem::InitializeViewer() {
  // create a ccptr
  ccptr pCC{new CentralControl(mNh, mNhPrivate, -1, eSystemState::SERVER,
                               nullptr, nullptr)};
