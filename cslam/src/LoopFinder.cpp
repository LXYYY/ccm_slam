...
void LoopFinder::Run() {
  while (1) {
    ...
    if (CheckKfQueue() &&
        !mpMap->isMergeStepGBA())  // should either way not close loops while
                                   // map merging, but who knows, maybe buffer
                                   // is full or sth like this...
#else
    if (CheckKfQueue())
#endif
    {
...
bool LoopFinder::DetectLoop() {
  ...
  const vector<kfptr> vpConnectedKeyFrames =
  ...
  vector<kfptr> vpCandidateKFs =
      mpKFDB->DetectLoopCandidates(mpCurrentKF, minScore * 0.8);
  ...
  vector<ConsistentGroup>
      vCurrentConsistentGroups;  // pair <set<KF*>,int> --> int counts
                                 // consistent groups found for this group
  ...
  for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
    ...
    for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++) {
      ...
      for (set<kfptr>::iterator sit = spCandidateGroup.begin(),
      ...
      if (bConsistent) {
        ...
        if (!vbConsistentGroup[iG]) {
          ...
          vbConsistentGroup[iG] =
              true;  // this avoids to include the same group more than once
        }
        if (nCurrentConsistency >= mnCovisibilityConsistencyTh &&
            !bEnoughConsistent) {
          mvpEnoughConsistentCandidates.push_back(pCandidateKF);
          bEnoughConsistent =
              true;  // this avoids to insert the same candidate more than once
        }
      }
    }
...
bool LoopFinder::ComputeSim3() {
  ...
  for (int i = 0; i < nInitialCandidates; i++) {
    ...
    // vvpMapPointMatches[KF candidate index][CurKF MP-IDc] = best matching MP
    // of candidate KF for MP-IDc of CurKF
    ...
    if (nmatches < params::opt::mMatchesThres) {
      ...
      pSolver->SetRansacParameters(params::opt::mProbability,
                                   params::opt::mMinInliers,
                                   params::opt::mMaxIterations);
  ...
  while (nCandidates > 0 && !bMatch) {
    for (int i = 0; i < nInitialCandidates; i++) {
      ...
      cv::Mat Scm = pSolver->iterate(params::opt::mSolverIterations, bNoMore,
                                     vbInliers, nInliers);
      ...
      // all correspondences
      if (!Scm.empty()) {
        vector<mpptr> vpMapPointMatches(vvpMapPointMatches[i].size(),
                                        static_cast<mpptr>(NULL));
  ...
  for (vector<kfptr>::iterator vit = vpLoopConnectedKFs.begin();
    ...
    for (size_t i = 0, iend = vpMapPoints.size(); i < iend; i++) {
      ...
      if (pMP) {
        if (!pMP->isBad() &&
            pMP->mLoopPointForKF_LC != mpCurrentKF->mId)  // ID Tag
        {
  ...
  // so far, mvpCurrentMatchedPoints[CurKF MP-IDc] = best matching MP of MatchKF
  // for MP-IDc of CurKF
  ...
  // (also in LC case, e.g. because of a previous loop that established the
  // connection), but this method finds another descriptor in CurKF that fits
  // better. Than later when adding/replacing MPs, this MP is observed twice by
  // CurKF, since the ids of new and exisitng observation don't match -- need to
  // catch this
...
void LoopFinder::CorrectLoop() {
  ...
  if (bProblem) {
    std::cout << __func__ << ":" << __LINE__ << "LC Module " << mpCC->mClientId
              << ": Rejecting Loop Closure because GBA is active" << std::endl;
    return;
    // below can cause deadlock - merging has locked GBA and wants to lock
    // PlaceRec, PlaceRec waits for GBA to get available
    //        std::cout << __func__ << ":" << __LINE__ << "LC Module " <<
    //        mpCC->mClientId << ": Waiting for GBA to be able to Start" <<
    //        std::endl; while(mpMap->isNoStartGBA())
    //        {
    //            usleep(params::timings::miLockSleep);
    //        }
  ...
  if (spCC.size() != mpMap->msuAssClients.size())
    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": "
            "spCC.size() != mpMap->msuAssClients.size()"
         << endl;
  ...
  for (set<ccptr>::iterator sit = spCC.begin(); sit != spCC.end(); ++sit) {
    ...
    if (pCC->mClientId > 3)
      cout
          << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": "
             "associated ClientId out of bounds ("
          << pCC->mClientId << ")" << endl;
    if (!(mpMap->msuAssClients.count(pCC->mClientId)))
      cout
          << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": "
             "associated ClientId in pCC but not in msuAssClients"
          << endl;
    switch (pCC->mClientId) {
      case (static_cast<size_t>(0)):
        if (b0)
          cout << "\033[1;31m!!! ERROR !!!\033[0m In "
                  "\"LoopFinder::CorrectLoop()\": associated ClientId found "
                  "twice"
               << endl;
        b0 = true;
        while (!pCC->LockComm()) {
      ...
      default:
        cout << "\033[1;31m!!! ERROR !!!\033[0m In "
                "\"LoopFinder::CorrectLoop()\": associated ClientId out of "
                "bounds"
             << endl;
    }
  }
  ...
  //    g2o::Sim3
  //    g2oScur(Converter::toMatrix3d(Rcur),Converter::toVector3d(tcur),1.0);
  ...
  for (vector<kfptr>::iterator vit = mvpCurrentConnectedKFs.begin(),
  ...
  for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
    ...
    for (size_t iMP = 0, endMPi = vpMPsi.size(); iMP < endMPi; iMP++) {
      ...
      if (pMPi->mCorrectedByKF_LC == mpCurrentKF->mId) continue;
  ...
  for (vector<kfptr>::iterator vit = mvpCurrentConnectedKFs.begin(),
    ...
    for (vector<kfptr>::iterator vit_prev = vpPreviousNeighbors.begin(),
                                 vend_prev = vpPreviousNeighbors.end();
         vit_prev != vend_prev; vit_prev++) {
      LoopConnections[pKFi].erase(*vit_prev);
    }
    for (vector<kfptr>::iterator vit2 = mvpCurrentConnectedKFs.begin(),
  ...
  // Optimize graph
  Optimizer::OptimizeEssentialGraphLoopClosure(mpMap, mpMatchedKF, mpCurrentKF,
  ...
  mpMap->mpThreadGBA =
      new thread(&LoopFinder::RunGBA, this, nLoopKF, sChangedKFs);
...
void LoopFinder::SearchAndFuse(const KeyFrameAndPose& CorrectedPosesMap,
                               std::vector<mpptr> vpLoopMapPoints) {
...
void LoopFinder::ResetIfRequested() {
  ...
  if (mbResetRequested) {
    ...
    for (vector<kfptr>::iterator vit = vpKFs.begin(); vit != vpKFs.end();
         ++vit) {
      kfptr pKF = *vit;
      mpKFDB->erase(pKF);
    }
...
void LoopFinder::RunGBA(idpair nLoopKF, set<idpair> sChangedKFs) {
  ...
  Optimizer::MapFusionGBA(mpMap, mpCC->mClientId, params::opt::mGBAIterations,
                          &(mpMap->mbStopGBA), nLoopKF, true);

  //    gettimeofday(&tNow,NULL);
  //    dEl = (tNow.tv_sec - tStart.tv_sec) + (tNow.tv_usec - tStart.tv_usec) /
  //    1000000.0; cout << "Optimization Time: Agents|KFs|MPs|Time: " <<
  //    mpMap->GetCCPtrs().size() << "|" << mpMap->GetAllKeyFrames().size() <<
  //    "|" << mpMap->GetAllMapPoints().size() << "|" << dEl << endl;

  set<ccptr> spCC = mpMap->GetCCPtrs();

  //    cout << "\033[1;33m!!! WARN !!!\033[0m Client " << mpCC->mClientId << ":
  //    In \"LoopFinder::CorrectLoop()\": fix GBA loop KF num when fixing id
  //    tags" << endl;
...
#endif
  {
    ...
    list<kfptr> lpKFtoCheck(mpMap->mvpKeyFrameOrigins.begin(),
    ...
    while (!lpKFtoCheck.empty()) {
      ...
      for (set<kfptr>::const_iterator sit = sChilds.begin();
           sit != sChilds.end(); sit++) {
        kfptr pChild = *sit;
        if (pChild->mBAGlobalForKF != nLoopKF) {
          ...
          if (!(pChild->mTcwGBA.dims >= 2))
            std::cout << COUTERROR << " KF" << pChild->mId.first << "|"
                      << pChild->mId.second << ": !(pChild->mTcwGBA.dims >= 2)"
                      << std::endl;
#endif
      ...
      if (!(pKF->mTcwGBA.dims >= 2))
        std::cout << COUTERROR << " KF" << pKF->mId.first << "|"
                  << pKF->mId.second << ": !(pKF->mTcwGBA.dims >= 2)"
                  << std::endl;
#endif
      ...
      if (!(pKF->mTcwBefGBA.dims >= 2))
        std::cout << COUTERROR << " KF" << pKF->mId.first << "|"
                  << pKF->mId.second << ": !(pKF->mTcwBefGBA.dims >= 2)"
                  << std::endl;
#endif
...
// If optimized by Global BA, just update
        ...
        if (!(pMP->mPosGBA.dims >= 2))
          std::cout << COUTERROR << " MP" << pMP->mId.first << "|"
                    << pMP->mId.second << ": !(pMP->mPosGBA.dims >= 2)"
                    << std::endl;
#endif
        ...
        if (!pRefKF) {
          cout << "\033[1;31m!!! ERROR !!!\033[0m In "
                  "\"LoopFinder::CorrectLoop()\": pRefKf is nullptr"
               << endl;
          continue;
        }

        if (pRefKF->mBAGlobalForKF != nLoopKF) continue;

#ifdef DEBUGGING2
        if (!(pRefKF->mTcwBefGBA.dims >= 2)) {
          std::cout << COUTERROR << " KF" << pRefKF->mId.first << "|"
                    << pRefKF->mId.second << ": !(pRefKF->mTcwBefGBA.dims >= 2)"
                    << std::endl;
          std::cout << "bad? " << (int)pRefKF->isBad() << std::endl;
          std::cout << "mBAGlobalForKF: " << pRefKF->mBAGlobalForKF.first << "|"
                    << pRefKF->mBAGlobalForKF.second << std::endl;
          std::cout << "nLoopKF: " << nLoopKF.first << "|" << nLoopKF.second
                    << std::endl;
          //                    kfptr pKFp = pRefKF->GetParent();
          //                    std::cout << "Parent " << pKFp->mId.first << "|"
          //                    << pKFp->mId.second << " -- bad: " <<
          //                    (int)pKFp->isBad() << std::endl;

          std::cout << "KF Lineage: " << std::endl;
          kfptr pKFp = pRefKF->GetParent();
          while (pKFp) {
            std::cout << "--> " << pKFp->mId.first << "|" << pKFp->mId.second
                      << " -- bad: " << (int)pKFp->isBad() << std::endl;
    ...
    //        {
    //            cout << COUTFATAL << "Attempt to UnLock MapUpdate -- was not
    //            locked" << endl; throw estd::infrastructure_ex();
    //        }
  ...
  //    std::cout << __func__ << ":" << __LINE__ << ": call setFinishedGBA()" <<
  //    std::endl;
  mpMap->setFinishedGBA();
  //    LLL
  ...
  for (set<ccptr>::iterator sit = spCC.begin(); sit != spCC.end(); ++sit) {
    ...
    //        std::cout << __func__ << ":" << __LINE__ << ": Unlock Mapping " <<
    //        pCC->mClientId << std::endl;

    pCC->UnLockMapping();
    //        if(!pCC->UnLockMapping())
    //        {
    //            cout << COUTFATAL << "Attempt to UnLock Comm -- was not
    //            locked" << endl; throw estd::infrastructure_ex();
    //        }
