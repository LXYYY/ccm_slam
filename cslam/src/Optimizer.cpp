...
void Optimizer::GlobalBundleAdjustemntClient(mapptr pMap, size_t ClientId,
                                             int nIterations, bool* pbStopFlag,
                                             const idpair nLoopKF,
                                             const bool bRobust) {
  vector<kfptr> vpKFs = pMap->GetAllKeyFrames();
  vector<mpptr> vpMP = pMap->GetAllMapPoints();
  BundleAdjustmentClient(vpKFs, vpMP, ClientId, nIterations, pbStopFlag,
                         nLoopKF, bRobust);
}

void Optimizer::BundleAdjustmentClient(const vector<kfptr>& vpKFs,
                                       const vector<mpptr>& vpMP,
                                       size_t ClientId, int nIterations,
                                       bool* pbStopFlag, const idpair nLoopKF,
                                       const bool bRobust) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKF->mId.first >= IDRANGE) {
      cout
          << "\033[1;31m!!!!! ERROR !!!!!\033[0m #1: In "
             "\"Optimizer::LocalBundleAdjustment(...)\": KF index out of bounds"
          << endl;
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    if (pMP->mId.first >= IDRANGE) {
      cout
          << "\033[1;31m!!!!! ERROR !!!!!\033[0m #1: In "
             "\"Optimizer::LocalBundleAdjustment(...)\": MP index out of bounds"
          << endl;
      throw infrastructure_ex();
    }
    ...
    int nEdges = 0;
    // SET EDGES
    for (map<kfptr, size_t>::const_iterator mit = observations.begin();
         mit != observations.end(); mit++) {
      kfptr pKF = mit->first;
      if (pKF->isBad()) continue;

      if (pKF->mId.first >= IDRANGE) {
        cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m #1: In "
                "\"Optimizer::LocalBundleAdjustment(...)\": KF index out of "
                "bounds"
             << endl;
        throw infrastructure_ex();
      }
      ...
      if (pKF->mvuRight[mit->second] < 0) {
        ...
        e->setVertex(1,
                     dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                         optimizer.vertex(Optimizer::GetID(pKF->mId, true))));
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(
        optimizer.vertex(Optimizer::GetID(pMP->mId, false)));
...
void Optimizer::LocalBundleAdjustmentClient(kfptr pKF, bool* pbStopFlag,
                                            mapptr pMap, size_t ClientId,
  ...
  for (list<kfptr>::iterator lit = lLocalKeyFrames.begin(),
                             lend = lLocalKeyFrames.end();
       lit != lend; lit++) {
    vector<mpptr> vpMPs = (*lit)->GetMapPointMatches();
    for (vector<mpptr>::iterator vit = vpMPs.begin(), vend = vpMPs.end();
         vit != vend; vit++) {
      mpptr pMP = *vit;
      if (pMP)
        if (!pMP->isBad())
  ...
  for (list<mpptr>::iterator lit = lLocalMapPoints.begin(),
                             lend = lLocalMapPoints.end();
       lit != lend; lit++) {
    map<kfptr, size_t> observations = (*lit)->GetObservations();
    for (map<kfptr, size_t>::iterator mit = observations.begin(),
  ...
  for (list<kfptr>::iterator lit = lFixedCameras.begin(),
        ...
        if (pKFi->mvuRight[mit->second] < 0) {
          ...
          e->setVertex(
              1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                     optimizer.vertex(Optimizer::GetID(pKFi->mId, true))));
          e->setMeasurement(obs);
  ...
  for (list<mpptr>::iterator lit = lLocalMapPoints.begin(),
    ...
    if (pMP->isBad()) {
      ...
      if (pMPcheck) {
        cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m " << __func__ << ":"
             << __LINE__ << " MP bad but not erased from map" << endl;
...
void Optimizer::MapFusionGBA(mapptr pMap, size_t ClientId, int nIterations,
                             bool* pbStopFlag, idpair nLoopKF,
                             const bool bRobust) {
  stringstream* ss;
  ss = new stringstream;
  for (set<size_t>::iterator sit = pMap->msuAssClients.begin();
       sit != pMap->msuAssClients.end(); ++sit)
    *ss << *sit << ";";
  cout << "--> Optimizing Map " << pMap->mMapId << " -- Contains Agents "
       << ss->str() << endl;
  ...
  if (pMap->mvpKeyFrameOrigins.empty()) {
    cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m " << __func__ << __LINE__
         << " pMap->mvpKeyFrameOrigins.empty()" << endl;
    throw infrastructure_ex();
  }
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    vertices++;
    ;
    if (pKF->mUniqueId > maxKFid) maxKFid = pKF->mUniqueId;
  }
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    for (map<kfptr, size_t>::const_iterator mit = observations.begin();
      ...
      e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                          optimizer.vertex(pKF->mUniqueId)));
      e->setMeasurement(obs);
      const float& invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];
      e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
  ...
  std::cout << "------> vertices|edges: " << vertices << "|" << edges
            << std::endl;
  ...
  std::cout << "------> Optimization Time: " << elapsed_seconds.count()
            << " [s]" << std::endl;
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    g2o::VertexSE3Expmap* vSE3 =
        static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mUniqueId));
    g2o::SE3Quat SE3quat = vSE3->estimate();
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    g2o::VertexSBAPointXYZ* vPoint =
        static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mUniqueId));
    ...
    if (nLoopKF == zeropair) {
      ...
      if (vPoint->estimate().rows() != 3 || vPoint->estimate().cols() != 1) {
        std::cout << COUTWARN
                  << "vPoint->estimate().rows() != 3 || "
                     "vPoint->estimate().cols() != 1"
                  << std::endl;
        continue;
      }
      Converter::toCvMat(vPoint->estimate()).copyTo(pMP->mPosGBA);
...
int Optimizer::OptimizeSim3(kfptr pKF1, kfptr pKF2,
                            std::vector<mpptr>& vpMatches1, g2o::Sim3& g2oS12,
                            const float th2, bool bFixScale) {
  g2o::SparseOptimizer optimizer;
...
void Optimizer::OptimizeEssentialGraphLoopClosure(
    mapptr pMap, kfptr pLoopKF, kfptr pCurKF,
  ...
  // Set Loop edges
  for (map<kfptr, set<kfptr> >::const_iterator mit = LoopConnections.begin(),
    ...
    for (set<kfptr>::const_iterator sit = spConnections.begin(),
                                    send = spConnections.end();
         sit != send; sit++) {
      if ((*sit)->isBad()) continue;
      const size_t nIDj = (*sit)->mUniqueId;

      if ((nIDi != pCurKF->mUniqueId || nIDj != pLoopKF->mUniqueId) &&
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    for (set<kfptr>::const_iterator sit = sLoopEdges.begin(),
                                    send = sLoopEdges.end();
         sit != send; sit++) {
      kfptr pLKF = *sit;
    ...
    for (vector<kfptr>::const_iterator vit = vpConnectedKFs.begin();
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    g2o::VertexSim3Expmap* VSim3 =
        static_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(pKFi->mUniqueId));
...
void Optimizer::OptimizeEssentialGraphMapFusion(
    mapptr pMap, kfptr pLoopKF, kfptr pCurKF,
    const map<kfptr, set<kfptr> >& LoopConnections, const bool& bFixScale) {
  // Setup optimizer
  g2o::SparseOptimizer optimizer;
