...
class ClientHandler : public boost::enable_shared_from_this<ClientHandler> {
 ...
 public:
  ClientHandler(ros::NodeHandle Nh, ros::NodeHandle NhPrivate, vocptr pVoc,
                dbptr pDB, mapptr pMap, size_t ClientId, uidptr pUID,
                eSystemState SysState, const string& strCamFile,
                viewptr pViewer, bool bLoadMap = false);
#ifdef LOGGING
  ...
  bool mbLoadedMap = false;  // indicates that map for this client was loaded
                             // from a file (only works for client 0)
